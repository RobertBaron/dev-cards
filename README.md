...
You need git to clone the `dev-cards` repository. You can get git from [here][git].

We also use a number of Node.js tools to initialize and test `dev-cards`. You must have Node.js
...
`dev-cards` changes this location through the `.bowerrc` file. Putting it in the `app` folder
...
The `dev-cards` app comes preconfigured with unit tests. These are written in [Jasmine][jasmine],
...
The `dev-cards` app comes with end-to-end tests, again written in [Jasmine][jasmine]. These tests
...
up-to-date. The `dev-cards` project is configured to do this automatically before running the
...
The `dev-cards` project supports loading the framework and application scripts asynchronously.
...
The `dev-cards` project comes preconfigured with a local development web server. It is a Node.js
...
your repository and execute scripts such as building the app or running tests. The `dev-cards`
...
AngularJS seed project from [angularjs.org][angularjs].
...
[repo]: git@github.com:RobertBaron/dev-cards.git